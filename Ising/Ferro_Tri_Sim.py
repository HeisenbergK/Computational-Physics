...
def make_whole_simulation(T):
    model = Ising(T, lattice, iterations, mesh="tri", prop="ferro", plots=False)
...
if __name__ == "__main__":
    ...
    T_sample = linspace(0.5, 5.5, temperature_points)
    ...
    df.to_csv(
        f"Ising/Results/Ferro_Tri_{lattice}_{iterations}_Simulation_{temperature_points}.csv",
        index=False,
    )
