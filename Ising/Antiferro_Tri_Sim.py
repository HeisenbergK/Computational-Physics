...
def make_whole_simulation(T):
    model = Ising(T, lattice, iterations, prop="antiferro", mesh="tri", plots=False)
...
if __name__ == "__main__":
    ...
    T_sample = linspace(0.3, 4.0, temperature_points)
    ...
    df.to_csv(
        f"Ising/Results/Antiferro_Tri_{lattice}_{iterations}_Simulation_{temperature_points}.csv",
        index=False,
    )
